...
def findNPeaks2(gray, kernel, pMin, pMax, n, minThresh=0, margin=1, ignorePAtMax=True, offset=(0,0), maxIter=40, drawImg=None, drawInvalidPeaks=False):
...
def findNPeaksWithBlur(gray, kernel, pMin, pMax, n, blurKernelSize, minThresh=0, margin=1, offset=(0,0), maxIter=20, drawImg=None, drawInvalidPeaks=False):
...
def meanShift(gray, center, kernel, maxIter=20, drawImg=None):
...
def maxShift(gray, center, kernel, maxIter=20, drawImg=None):
    assert kernel.shape[0] % 2 == 1, "Kernel size must be uneven"
    radius = int(kernel.shape[0]/2)
    maxPrev = 0
    for i in range(maxIter+1):
